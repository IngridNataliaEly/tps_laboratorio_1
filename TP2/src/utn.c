...
int utn_getShort(short* pResultado,char* mensaje,char* mensajeError,int minimo,int maximo,int reintentos)
{
	...
	if(pResultado != NULL && mensaje != NULL && mensajeError != NULL && minimo <= maximo && reintentos>=0)
	{
		...
		{
			printf("%s", mensaje);
			if(getShort(&bufferInt)==0 && bufferInt>=minimo && bufferInt<=maximo)
			{
...
int getNombre(char* pResultado,int size)
{
	...
	{
		if(myGets(buffer, sizeof(buffer))==0 && esNombre(buffer, sizeof(buffer))!=0 && strnlen(buffer, sizeof(buffer))<=size)
		{
